module StatsAlertsHelper
  ...
  def alerts_by_group_alert_donut
    data = select_range_of_dates_for(GroupAlert.includes(:type_alerts)).joins(:alerts).group("group_alerts.title").count
    pie_chart data, donut: true, legend: "bottom", library: basic_opts('% By alert group')
  end

  def alerts_by_type_alert
    data = select_range_of_dates_for(Alert.includes(:type_alert)).joins(:type_alert).group("type_alerts.id").count.map{|type_alert_id, count| [type_alert_id ? truncate_string(TypeAlert.find(type_alert_id).name) : "Unassigned", count] } # https://github.com/ankane/chartkick/issues/19
  ...
  def top_10_solutions
    data = select_range_of_dates_for(Alert.includes(:issue).all_resolved).joins(:issue).group("issues.id").count.map{|issue_id, count| [issue_id ? truncate_string(Issue.find(issue_id).name) : "Unassigned", count] } # https://github.com/ankane/chartkick/issues/19
    top_10_data = data.sort {|a, b| b[1] <=> a[1]}.first 10 # https://stackoverflow.com/questions/9615850/ruby-sort-array-of-an-array
    
    bar_chart top_10_data, id: "Top_10_solutions", library: basic_opts('Top 10 Solutions')
  end
