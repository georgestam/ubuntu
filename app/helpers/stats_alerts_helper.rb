module StatsAlertsHelper
  ...
  def total_alerts_in_time
    alerts_created = by_week(Alert.all_not_hidden, "created_at")
    alerts_resolved = by_week(Alert.all_not_hidden, "resolved_at")
  ...
  def alerts_by_time
    data = GroupAlert.all.map { |group_alert| 
      {name: group_alert.title, data: by_week(group_alert.alerts.all_not_hidden, "alerts.created_at")} 
    }
    column_chart data, legend: "bottom", xtitle: "", ytitle: "", library: basic_opts('Number of Alerts (Alerts Group)')
  end 
  
  # TODO: make this graph with all_not_hidden scope
  ...
  def alerts_by_type_alert
    data = select_range_of_dates_for(Alert.all.all_not_hidden).joins(:type_alert).group("type_alerts.id").count.map{|type_alert_id, count| [type_alert_id ? truncate_string(TypeAlert.find(type_alert_id).name) : "Unassigned", count] } # https://github.com/ankane/chartkick/issues/19
