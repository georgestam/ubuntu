...
if Rails.env.in?(%w(test development))
  ...
  CarrierWave::Uploader::Base.descendants.each do |descendant|
    ...
    descendant.class_eval do
      ...
      def cache_dir
        Rails.root.join('public', 'uploads', Rails.env.to_s, 'tmp')
      end

      def store_dir
        Rails.root.join('public', 'uploads', Rails.env.to_s, model.class.to_s.underscore, mounted_as.to_s, model.id.to_s)
      end
    end
  end
